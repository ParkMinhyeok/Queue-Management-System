...
* 우선순위로 인해 중간까지 추론될 경우 중간 데이터를 클라이언트로 전송한다.
#### 통신 모듈 :
* WiFi 프로토콜을 통해 클라이언트와 서버 간의 실시간 데이터를 교환한다.
#### 큐 관리 시스템 :
* 우선순위와 예상 처리 시간을 계산하여 작업을 스케줄링한다.
* 자세한 내용은 구현 세부사항에서 소개

### 구현 세부사항
* 현재 서버에서 처리되는 클라이언트보다 높은 우선순위 클라이언트의 요청이 들어올 경우 처리되고 있던 클라이언트의 처리를 중단하고 우선순위가 높은 새로운 클라이언트의 요청을 처리한다.
* 이때 중단된 데이터는 해당 클라이언트로 반환되어 온디바이스 환경에서 나머지 처리를 진행하게된다.
* 예상 처리 시간을
